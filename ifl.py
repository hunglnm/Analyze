class IFL:
    ...
    def __init__(self):
        ...
        self.vrrp_track=''
        self.vrrp_reduce =''
        self.isis_authen= False
        self.trust_upstream = False

    def showdata(self):
        attrs = vars(self)
        print(IFL.Hostname,',',','.join("%s: %s" % item for item in list(attrs.items())))
    ...
    def insert(self, cursor):
        add_ifl = ("INSERT INTO IFL "
                   ...
                   "CCC_Name,RRPP,RRPP_vsi_list,VSI_encap,FF_out,DHCP_GW,DF_classifier,VRRP_track,VRRP_reduce,"
                   "ISIS_authen,Trust_upstream )"
                   ...
                   "DF_classifier=VALUES(DF_classifier),VRRP_track=VALUES(VRRP_track),VRRP_reduce=VALUES(VRRP_reduce)"
                   ",ISIS_authen=VALUES(ISIS_authen),Trust_upstream=VALUES(Trust_upstream) ")
        data_ifl = (self.IFD,IFL.Hostname,self.Unit,self.Unit1,self.Description,self.Service,
                    ...
                    self.df_classifier,self.vrrp_track,self.vrrp_reduce,self.isis_authen,self.trust_upstream)
